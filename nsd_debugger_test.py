...
from utils import (
    ...
    extract_code_blocks, STARTING_CODE
)
...
class FineGrainedTracer:
    ...
    def get_trace_summary(self, testcase: Dict) -> str:
        ...
        Args:
            testcase: 必须包含 'input' 和 'expected_output'，用于对比结果偏差
        """
        if not self.traces:
...
def evaluate_with_trace(code: str, func_name: str, testcase: Dict, 
    ...
    try:
        ...
        return False, {'error': f"SyntaxError: {e}"}, None
    ...
    try:
        ...
        try:
            ...
            # ✅ 修复：将返回值注入testcase，确保追踪器知道结果
            testcase['actual_output'] = result  # 👈 关键：在此处设置
            ...
            #         return len(a) == len(b) and all(is_close(x, y, rel_tol, abs_tol) for x, y in zip(a, b))
            ...
            # # 使用近似比较替代严格相等
            # passed = is_close(result, expected)
...
# openai.api_base = LLM_BASE_URL
    ...
    def _build_debug_prompt(self, func_code: str, func_name: str,
                           ...
                           error_info: Dict) -> str:
        """构建调试prompt"""
        prompt = f"""你是Python调试专家。请基于执行轨迹定位bug并修复代码。
